...
disp('Make sphCoords.mat and TOA_table.mat');
result_dir= './mat';
...
for nm=M
    ...
    for nq=Q
        ...
        mk_toa_table_full(input_mic_array,mic_array_origin,search_range,Fs,c);
        
        dest_filename= sprintf('sphCoords_Q%d.mat',nq);
        movefile('sphCoords.mat',[result_dir '/' dest_filename]);
        dest_filename= sprintf('TOA_table_M%d_Q%d.mat',nm,nq);
        movefile('TOA_table.mat',[result_dir '/' dest_filename]);
    end
end
...
for nm=M
    ...
    for nq=Q
        ...
        load(load_filename);
        load_filename= sprintf('%s/TOA_table_M%d_Q%d.mat',mat_dir,nm,nq);
        load(load_filename);
        ...
        TOA_table= cast(TOA_table,'single')';
        dest_filename= sprintf('TOA_table_M%d_Q%d.bin',nm,nq);
        fid= fopen(dest_filename,'wb');
        fwrite(fid,TOA_table,'single','ieee-le');    % For Linux
        fclose(fid);
        movefile(dest_filename,[result_dir '/' dest_filename]);
    end
end