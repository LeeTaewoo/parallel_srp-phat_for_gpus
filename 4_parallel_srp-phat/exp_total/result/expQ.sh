...
#
# Out: ./log/expQ/exp_M?_Q?_TD?_CC?_SRP?_FD?_SRP?.log
#
# Reference:
...
for q in 3888 97200 388800; do
    ...
    echo "exp_Q M=""$m"" Q=""$q"" FDGPU""$fdgpu"
		rm -f ../../exp
		CheckError
		# copy exe
...
#			nvprof --print-gpu-trace ./exp $dir 10 >> ./exp_total/result/"Detailed_exp_M""$m""_Q""$q""_TD0_CC0_SRP0_FD1_SRP""$fdgpu"".log" 2>&1
			CheckError
			cd ./exp_total/result
			CheckError
		done	
  ...
  CheckError
	...
	for tdcc in 3; do       #3=Proposed
	  for tdsrp in 43; do   #43=Proposed
	    echo "exp_Q M=""$m"" Q=""$q"" TDGPU ""CC=""$tdcc"" SRP=""$tdsrp"
	    rm -f ../../exp
	    CheckError
	    # copy exe
